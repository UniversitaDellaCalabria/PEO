...
def migra_domanda_vecchia(matricola='', debug=1):
    bando_vecchio = Bando.objects.all()[2]
    domanda_vecchia = DomandaBando.objects.get(bando=bando_vecchio,
                                               dipendente__matricola=matricola)

    bando_nuovo = Bando.objects.last()
    domanda_nuova = DomandaBando.objects.get(bando=bando_nuovo,
                                             dipendente__matricola=matricola)

    for i in domanda_vecchia.modulodomandabando_set.filter(disabilita=False):
        nd = i.__dict__
        del(nd['_state'])
        ...
        indicatore_ponderato = IndicatorePonderato.objects.get(bando=bando_nuovo,
