...
from unical_template.utils import (differenza_mesi_ad_oggi_aru,
...
class PunteggioDomandaBando(object):
    ...
    def calcola_punteggio_anzianita_automatico(self):
        ...
        # Quanti mesi sono passati, ad oggi, dalla data limite imposta dal bando
        mesi_da_limite_bando =  differenza_mesi_ad_oggi_aru(data_limite)
        
        # I mesi di anzianità da considerare per il bando in questione
        anzianita_considerata = mesi_anzianita - mesi_da_limite_bando
        unita_temporale = "m"
        ...
        if self.bando.punteggio_anzianita_servizio_set.first():
            ...
            if unita_temporale=="m":
                punteggio += punteggio_categoria*anzianita_considerata
            elif unita_temporale=="y":
                punteggio += punteggio_categoria*(anzianita_considerata/12)
        
            # Se non è stata effettuata alcuna progressione e 
            # l'anzianità è maggiore o uguale alla soglia di bonus
            if not self.dipendente.get_ultima_progressione_mesi() and \
            anzianita_considerata >= 12*self.bando.agevolazione_soglia_anni:
                punteggio = punteggio*self.bando.agevolazione_fatmol
        
            elif self.dipendente.get_ultima_progressione_mesi():
                # Mesi di permanenza nell'ultima categoria economica ad oggi
                permanenza_cat_eco = self.dipendente.get_ultima_progressione_mesi()
                
                # Mesi nell'ultima categoria economica trascorsi alla data limite del bando
                permanenza_considerata = permanenza_cat_eco - mesi_da_limite_bando
        
                #Se è soddisfatta la condizione del bando, si applica il bonus di moltiplicazione
                if permanenza_considerata >= 12*self.bando.agevolazione_soglia_anni:
                    
                    # Poichè i mesi di permanenza sono stati già considerati prima
                    # la moltiplicazione la faccio per 'fatmol-1' e aggiungo il 'plus'
                    bonus = punteggio_categoria*permanenza_considerata*(self.bando.agevolazione_fatmol-1)
                    punteggio += bonus
        return punteggio
...
class PunteggioModuloDomandaBando(object):
    ...
    def get_durata_int(self,
        ...
        inizio = termine = None
        ultima_progressione = self.domanda_bando.get_data_progressione()
        ...
        # Se nel form è stato inserito il campo "intervallo date IN RANGE Bando"    
            ...
            return differenza_mesi_ad_oggi_aru(inizio, now=termine)
